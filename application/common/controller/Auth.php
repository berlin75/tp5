 ...
 * 权限认证类
 * 功能特性：
 * 1，是对规则进行认证，不是对节点进行认证。用户可以把节点当作规则名称实现对节点进行认证。RBAC对节点进行认证
 *    $auth=new Auth();  $auth->check('规则名称','用户id')
 * 2，可以同时对多条规则进行认证，并设置多条规则的关系（or或者and）
 *    第三参数为and表示，用户需要同时具有规则1和规则2的权限。第三个参数为or，表示用户值需要具备其中一个条件即可。默认为or
...
/*
  ...
  -- bk_auth_rule，规则表，
  -- id:主键，
  -- name：规则唯一标识,需要认证的【模块名/控制器名/方法名】或【自定义规则】字符串类型 ，多个规则之间用,也可以填写一个 admin 或 * 或 guanliyuan 等！
    1.(Admin/Article/Add,Admin/Article/Edit,Admin/Article/Delete)
    2.Article
    3.(Article-Add-Edit-Delete)
  -- title：规则中文名称 status 状态：为1正常，为0禁用，condition：规则表达式，为空表示存在就验证，不为空表示按照条件验证
  如type为1，condition字段就可以定义规则表达式，将会用作正则表达式的规则来配合认证规则来认证用户
  -- ----------------------------
